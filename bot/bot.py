...
def strip_tool_call_leaks(text: str) -> str:
    """Удаляет orphan tool-call JSON в plain text (включая Llama-style с
    'parameters' и 'name' без 'arguments'), плюс strip иероглифов и фраз
    объявления процесса работы («я вызову инструмент X»)."""
    import re as _re

    # 0) Замеряем длину input — для placeholder-fallback если выкосим всё
    _input_len = len(text.strip())

    # 1) Fenced JSON блоки с "name"
    text = _re.sub(
        r"```(?:json)?\s*\{[^{}]*\"name\".*?\}\s*```",
        "",
        text,
    ...
    # 2) Голый top-level JSON {"name": "...", "arguments|parameters": {...}}
    # Покрывает оба варианта (Ollama spec: arguments, Llama-leak: parameters).
    text = _re.sub(
        r"\{[^{}]*\"name\"\s*:\s*\"[A-Za-z_][\w-]*\"[^{}]*\"(?:arguments|parameters)\"\s*:\s*\{(?:[^{}]|\{[^{}]*\})*\}[^{}]*\}",
        "",
        text,
        flags=_re.DOTALL,
    )
    # 3) Pre-tool-call narration — фразы вида «я вызову инструмент X», «сейчас
    # воспользуюсь web_search», «обращусь к функции», «нужно вызвать tool»
    narration_patterns = [
        r"[^.!?\n]*(?:вызову|вызовем|вызываю|выполню|выполняю|воспользуюсь|обращусь)\s+(?:инструмент(?:ом)?|функци[юей]|tool|web_search|websearch)[^.!?\n]*[.!?]?",
        r"[^.!?\n]*(?:Поскольку\s+вы\s+попросили|Поскольку\s+пользователь\s+просит)[^.!?\n]*[.!?]?",
        r"[^.!?\n]*(?:сейчас\s+поищу|сейчас\s+проверю)\s+(?:в\s+интернете|в\s+сети)[^.!?\n]*[.!?]?",
    ]
    for pat in narration_patterns:
        text = _re.sub(pat, "", text, flags=_re.IGNORECASE)

    # 4) Strip foreign-script symbols (CJK/Hangul/Hebrew/Arabic/Devanagari/Thai)
    foreign_chars = _re.compile(
        "["
        "\u4E00-\u9FFF"
        "\u3040-\u309F"
        "\u30A0-\u30FF"
    ...
    text = foreign_chars.sub("", text)

    # 5) Уберём лишние пробелы/пустые строки
    text = _re.sub(r"[ \t]{2,}", " ", text)
    text = _re.sub(r"\n{3,}", "\n\n", text).strip()

    # 6) Если из вменяемого input'а выкосили почти всё → placeholder
    if _input_len > 20 and len(text) < 5:
        return "У меня нет актуальных данных по этому запросу."
    return text
...
# --- patch_runner hookup (auto-injected by inject_patch_runner.py) ---
from pathlib import Path as _PR_Path
from patch_runner_handler import register_patch_runner as _pr_register
_pr_patches_dir = _PR_Path(__file__).parent.parent / "patches-incoming"
_pr_register(dp, bot, _pr_patches_dir, 263280027)
# --- end patch_runner hookup ---

# --- file_clone hookup (auto-injected by inject-file-clone.sh) ---
from pathlib import Path as _FC_Path
from file_clone_handler import register_file_clone as _fc_register
_fc_register(
    dp,
    repo_root=_FC_Path('/home/linuxserver/ai-assistant').expanduser().resolve(),
    bot_self_path=_FC_Path(__file__).resolve(),
    admin_id=263280027,
)
# --- end file_clone hookup ---


# --- broadcast hookup (auto-injected by inject-broadcast.sh) ---
from broadcast_handler import register_broadcast as _bc_register
async def _bc_list_recipients() -> list[int]:
    db = await _get_db()
    out: list[int] = []
    ...
    return out
async def _bc_is_admin(uid: int) -> bool:
...
async def chat_handler(msg: Message):
  ...
  # P3 fix: assignment в document handler ниже делает user_text локальной без этого
        ...
        # Юра msg 17477: auto-copy admin uploads of .sh/.py/.bash/.patch в
        # patches-incoming/ — для использования с /list_patches + /run_patch.
        if msg.document and await db_is_admin(user_id):
            try:
                _patch_exts = {".sh", ".py", ".bash", ".patch"}
                from pathlib import Path as _PR_Path
                _fname = (msg.document.file_name or "").lower()
                _ext = _PR_Path(_fname).suffix
                if _ext in _patch_exts:
                    _pr_inbox = _PR_Path(__file__).parent.parent / "patches-incoming"
                    _pr_inbox.mkdir(parents=True, exist_ok=True)
                    _pr_safe_name = "".join(c if c.isalnum() or c in "._- " else "_"
                                            for c in (msg.document.file_name or f"patch-{int(time.time())}{_ext}"))
                    _pr_target = _pr_inbox / _pr_safe_name.strip()
                    if _pr_target.exists():
                        _pr_target = _pr_inbox / f"{_pr_target.stem}-{int(time.time())}{_pr_target.suffix}"
                    _pr_tg_file = await bot.get_file(msg.document.file_id)
                    _pr_buf = await bot.download_file(_pr_tg_file.file_path)
                    _pr_target.write_bytes(_pr_buf.read())
                    _pr_size_kb = _pr_target.stat().st_size // 1024
                    log.info("patch saved to patches-incoming: %s (%s KB)", _pr_target, _pr_size_kb)
                    await msg.answer(
                        f"📥 Принят patch: <code>{_pr_target.name}</code>\n"
                        f"Сохранён в <code>patches-incoming/</code> ({_pr_size_kb} KB).\n\n"
                        f"Запустить: <code>/run_patch {_pr_target.name}</code>\n"
                        f"Список:    <code>/list_patches</code>"
                    )
            except Exception as e:
                log.warning("patch auto-save failed: %s", e)
        ...
        system = {
            ...
            "content": (
                ...
                "⚠️ САНКЦИЯ: если ты выдашь иероглиф (китайский/корейский/японский/иврит/арабский), автоматический пост-фильтр вырежет этот фрагмент. Если после фильтрации в ответе останется меньше нескольких символов — юзер вообще не получит твой ответ. Пиши строго кириллицей.\n"
                ...
                "ГЕНЕРАЦИЯ КАРТИНОК — ПАССИВНОЕ ПРАВИЛО (v8 PASSIVE_IMAGE_RULE 2026-05-18):\n"
                ...
                "АНТИ-ГАЛЛЮЦИНАЦИЯ ФИНАНСОВЫХ ДАННЫХ (v10 ANTI_HALLUCINATION_CURRENCY 2026-05-18):\n"
                ...
                "ОБЩЕЕ АНТИ-ГАЛЛЮЦИНАЦИОННОЕ ПРАВИЛО (v21 GENERAL_ANTI_HALLUCINATION 2026-05-18, ВАЖНО):\n"
                ...
                "ИСПОЛЬЗОВАНИЕ ЛИЧНОЙ АНКЕТЫ ЮЗЕРА (v14 PROFILE_AWARE_CONTEXT 2026-05-18, обязательно):\n"
        ...
        try:
            ...
            if output_dir.exists():
                # Anti-spurious-file guard (Юра msg 17435, 2026-05-17):
                # auto-ship docx/png только если юзер реально просил файл/документ.
                # Иначе модель over-eager (на "Супер!" создаёт супер.docx).
                import re as _re_intent
                _file_intent = _re_intent.compile(
                    r"(?i)(\bфайл\w*|\bдокумент\w*|docx|pdf|\bотч[её]т\w*|\bсохрани|\bподготовь|\bсоставь|\bсделай\s+(?:файл|документ|отч))"
                )
                _image_intent = _re_intent.compile(
                    r"(?i)(\bкартинк\w+|\bизображени\w+|\bрисунок\w*|\bphoto\b|\bimage\b|\bрендер\w*|\bсгенерируй|\bнарисуй)"
                )
                _user_text_for_intent = (user_text or "").strip()
                _ship_docx = bool(_file_intent.search(_user_text_for_intent))
                _ship_png = bool(_image_intent.search(_user_text_for_intent))
                _html_intent = _re_intent.compile(
                    r"(?i)(\bсайт\w*|\bстраниц\w*|\bлендинг\w*|\blanding\b|\bhtml\b|\bодностраничн\w*|\bвеб-?страниц\w*)"
                )
                _ship_html = bool(_html_intent.search(_user_text_for_intent))
                if not _ship_html:
                    log.info("html auto-ship skipped: no html-intent in user_text=%r",
                            _user_text_for_intent[:60])
                if not _ship_docx:
                    log.info("docx auto-ship skipped: no file-intent in user_text=%r",
                            _user_text_for_intent[:60])
                if not _ship_png:
                    log.info("png auto-ship skipped: no image-intent in user_text=%r",
                            _user_text_for_intent[:60])
                if _ship_docx:
                ...
                # P3c: HTML auto-send — generate_html_page output (Юра msg 17443)
                if _ship_html:
                    for html_file in output_dir.glob("*.html"):
                        if time.time() - html_file.stat().st_mtime < 60:
                            try:
                                await msg.answer_document(_FSInputFile(str(html_file)))
